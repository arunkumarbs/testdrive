...
public class WhatsApi {
    ...
    private List<Country> readCountries() throws WhatsAppException {
        ...
        //BufferedReader is1 = file.getClass()
    ...
    private void doLogin() throws InvalidKeyException, NoSuchAlgorithmException, IOException, InvalidKeySpecException, WhatsAppException, IncompleteMessageException, InvalidMessageException, InvalidTokenException, JSONException, EncodeException, DecodeException {
        ...
        int cnt = 0;
        poller = (MessagePoller)ThreadManager.createThreadForCurrentRequest(new MessagePoller(this));
        poller.start();
        do {
            try {
                Thread.sleep(100);
    ...
    private void doLogin(boolean flag) throws InvalidKeyException, NoSuchAlgorithmException, IOException, InvalidKeySpecException, WhatsAppException, IncompleteMessageException, InvalidMessageException, InvalidTokenException, JSONException, EncodeException, DecodeException {
        writer.resetKey();
        reader.resetKey();
        String resource = WHATSAPP_DEVICE + "-" + WHATSAPP_VER + "-" + PORT;
        byte[] data = writer.startStream(WHATSAPP_SERVER, resource);
        ...
        while (node != null) {
            ...
            try {
                ...
                if (tag == null) {
                    ...
                    log.info("Unknown/Unused tag (null) {}" + node.toString());
                    //sendAck(node);
                }
            } catch (IllegalArgumentException e) {
                tag = ProtocolTag.UNKNOWN;
                log.info("Unknown/Unused tag " + node.getTag());
                log.info("Sending ack anywat to: {}" + node.toString());
                //sendAck(node);
            }
